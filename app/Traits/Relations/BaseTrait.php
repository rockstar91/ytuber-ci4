...
trait BaseTrait
{
    ...
    public function _getRelations($tableName, $ids = null): array
    {
        ...
        // Устанавливаем displayModeOn у связанных моделей, если используется у родительской модели
        if(isset($this->displayMode) && $this->displayMode)
        {
            if(method_exists($builder, 'displayModeOn'))
                $builder->displayModeOn();
        }

        // хак для displayMode
        if( ! isset ($builder->displayMode) OR ! $builder->displayMode)
        {
            // Define the returns
