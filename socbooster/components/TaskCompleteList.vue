<template>
    <div>
        ...
        <div class="page-header page-header-light">
            <div class="page-header-content header-elements-md-inline">
                <div class="page-title d-flex">
                    <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Task Manager</span> -
                        Grid</h4>
        ...
        <!-- /page header -->
        <TaskListCompleteFilter></TaskListCompleteFilter>
        <div class="content">
...
<script>
    export default {
        ...
        mounted() {
            this.$root.$on('ytfinish', (c) => {
                try {
                    const regex = /(?:[?&]vi?=|\/embed\/|\/\d\d?\/|\/vi?\/|https?:\/\/(?:www\.)?youtu\.be\/)([^&\n?#]+)/g;
                    const m = regex.exec(this.tasks.data[0].owntask.url);
                    const str = String(String('ytbe' + this.tasks.data[0].owntask.time + this.user.balance) + String(m[1] + this.user.avatar));
                    const bcrypt = require('bcryptjs');
                    const balance = this.user.balance;
                    const task_id = this.tasks.data[0].owntask.id;
                    bcrypt.compare(str, c, function (err, result) {
                        if (result) {
                            const postarray = {
                                'mark': c,
                                'video_id': m[1],
        ...
        methods: {
            CheckHash(taskid, task_typename, task_typeid, social_typename, username, social_typeid) {
                ...
                const bcrypt = require('bcryptjs');
                const hash = bcrypt.hashSync(taskid + task_typename + task_typeid + social_typename + 'j34fs7vxSdfVCXs2dxf' + social_typeid, 10);
                    ...
                    if (this.openedWindow.closed) {
                        ...
                        if (this.closed == true) {
                            if (this.max >= this.time) {
                                ...
                                this.time = 0;
                                this.progress = false;
                                this.CheckHash(this.tasks.data[0].owntask.id,this.tasks.data[0].owntask.task_type.name,this.tasks.data[0].owntask.task_type.id,this.tasks.data[0].owntask.social_type.name,'j34fs7vxSdfVCXs2dxf',this.tasks.data[0].owntask.social_type.id);
                            } else {
                                clearInterval(this.interval);
            ...
            fetchData(page = 1) {
                ...
                this.page = page;
                // request to backend
                axios.get('/api/task_complete_list?page=' + page + '&tasktype=' + this.filter.tasktype + '&socialtype=' + this.filter.socialtype + '&status=' + this.filter.status + '&task_id=' + this.task_id)
                    .then((res) => {
                        this.tasks = res.data;
                        this.loading = false;
        ...
        components: {
            'TaskListCompleteFilter': require('./TaskListCompleteFilter.vue').default,
