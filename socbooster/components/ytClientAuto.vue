    ...
    <div>
        <div class="content">
            <loading :active.sync="loading"
                     :can-cancel="false"
                     :on-cancel="onCancel"
                     :is-full-page="true" loader="dots" color="green"><h2><i
                    class="icon-spinner11 spinner mr-2"></i>&nbsp;{{status_task}}... </h2></loading>
            <!-- Image grid -->
                            ...
                            <div class="d-flex-row" v-if="progress == false">
                                ...
                                <div class="d-flex" v-if="(task.owntask.custom_comment)">
                                    ...
                                    <input type="text" class="form-control" readonly=""
                                           :value="tasks.data[0].customcomment.text">
                                </div>
...
<script>
    export default {
            ...
            fetchData(page = 1) {
                ...
                axios.get('/api/get_video/')
                    .then((res) => {
                        ...
                        this.task = res.data;
                        this.loading = false;
                        console.log(this.task['id'],this.task['task_type_id'],this.task['user_id'],this.task, this.task['social_type_id'],this.task['autojoin']);
                        this.joinTask(this.task['id'],this.task['task_type_id'],this.task['user_id'],this.task, this.task['social_type_id'],this.task['autojoin']);
                    })
                    .catch((err) => {
                        console.log(err);
                        this.loading = false;
        ...
        components: {
            ...
            'YtClient': require('./ytClientAuto.vue').default,
