		...
		function CheckAdblock() {
			...
			if(adblock==1){
				$('.error').html(please_disable_adblock);
			}
		}
		...
		$( document ).ready(function() {
			...
			function WindowLocation(url) {
				...
				if(typeof video_window !== 'undefined') {
					video_window.close();
				}
				window.location = url;
			} 
			...
			var watched = 0;
				...
				function WatchVideo(){
					watched = 0;
					video_window = window.open('/client/wait');
					LoadVideo();
				}
				/*---------------*/
						...
						$.ajax({
							  ...
							  success: function(data){
							    if(typeof data.url !== 'undefined') {
									video_window.location = data.url;
									$(".countdownGroup").show();
									...
									$(".countdownGroup").hide();
									watched = 0;
									video_window.location = "/client/wait";
				...
				function WatchTimer(time, id, vid){
					...
					WatchTimerTimeout = setTimeout( function(){ 
						if(video_window.closed){
							...
							// Если about:blank или что-то в этом духе
							/*var hrefLength = false;
							try{
								if (video_window.location.href.length) {
									hrefLength = true;
                            ...
                            var h = MD5(vid + id + adblock + isbot + agent + ks); 
